...
function unix2human() {
    ...
    document.unix2beijing.resultBJ.value = UnixTimeToDate;
    //北美时间
    var d = new Date(document.unix2beijing.timestamp.value * 1000);
    var c = d.getTime() + d.getTimezoneOffset()*60000;
    dateObj = new Date(c+3600000*(-4));
    var UnixTimeToDateBM = dateObj.getFullYear() + '/' + formatDate(dateObj.getMonth() + 1) + '/' + formatDate(dateObj.getDate()) + ' ' + formatDate(dateObj.getHours()) + ':' + formatDate(dateObj.getMinutes()) + ':' + formatDate(dateObj.getSeconds());
    document.unix2beijing.resultAC.value = UnixTimeToDateBM;

    //泰国时间
    d = new Date(document.unix2beijing.timestamp.value * 1000);
    c = d.getTime() + d.getTimezoneOffset()*60000;
    dateObj = new Date(c+3600000*(+7));
    document.unix2beijing.resultTH.value = dateObj.getFullYear() + '/' + formatDate(dateObj.getMonth() + 1) + '/' + formatDate(dateObj.getDate()) + ' ' + formatDate(dateObj.getHours()) + ':' + formatDate(dateObj.getMinutes()) + ':' + formatDate(dateObj.getSeconds());
}
